terraform {
  ...
  # preflight checks that don't apply to Spaces.
  #
  ...
  # backend only reads those names, not SPACES_*).
  backend "s3" {
    bucket = var.bucket
    ...
    # State locking via the backend's native conditional-write lockfile; no
    # separate lock table is needed.
    use_lockfile = true

    # Disable the validation/lookups that don't apply to Spaces.
